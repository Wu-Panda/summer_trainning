...
public class Menu {
    ...
    private static final String PRODUCT_DATA_FILE = "../data/product.txt";
    private static final String MEMORY_DATA_FILE = "../data/memory.txt";
    ...
    private static void loadUserData() {
        try {
            ...
            if (file.exists()) {
                ...
                System.out.println("未找到用户数据,开始使用空用户列表.");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static void loadProductData() {
        try {
            File file = new File(PRODUCT_DATA_FILE);
            if (file.exists()) {
                Scanner scanner = new Scanner(file);
                while (scanner.hasNextLine()) {
                ...
                System.out.println("未找到商品数据,开始使用空商品列表.");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static void saveCheckoutInfo(String info) {
        try {
            FileWriter writer = new FileWriter(MEMORY_DATA_FILE, true);
            writer.write(info + "\n");
            writer.close();
    ...
    private static List<String> loadCheckoutInfo() {
        ...
        try {
            File file = new File(MEMORY_DATA_FILE);
            if (file.exists()) {
                Scanner scanner = new Scanner(file);
                while (scanner.hasNextLine()) {
