...
public class Users {
    ...
    public static void createGUI() {
        ...
        // 加载背景图片
        ImageIcon backgroundImage = new ImageIcon(Users.class.getResource("/MainBackground.jpg")); // 获取图片的位置
        ...
        changePasswordButton.addActionListener(e -> {
            ...
            if (result == JOptionPane.OK_OPTION) {
                ...
                if (currentPassword.isEmpty() || newPassword.isEmpty()) {
                    ...
                    boolean success = updatePassword(currentPassword, newPassword, LoginAction.id); // 调用数据库操作方法更新密码
                    if (success) {
        ...
        resetPasswordButton.addActionListener(e -> {
            // 在这里编写重置密码的逻辑代码
            boolean success = resetPassword("123", LoginAction.id); // 将密码重置为固定值 "123"，并传递当前登录用户的用户名
            if (success) {
        ...
        logoutButton.addActionListener(e -> {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                ...
                LoginAction.main(null); // 调用登录页面的主函数重新打开登录页面
            }
        });
        ...
        viewProductsButton.addActionListener(e ->{
            // 在这里编写登出的逻辑代码
            frame.dispose(); // 关闭当前窗口
            Shop.main(null); // 调用登录页面的主函数重新打开登录页面
        });
