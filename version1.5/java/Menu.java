...
public class Menu {
    ...
    private static void loadUserDataFromFile(Statement statement) {
        try (BufferedReader reader = new BufferedReader(new FileReader("../data/users.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                // 检查数据库中是否已存在相同的用户名
                String selectQuery = "SELECT username FROM " + USER_DATA_TABLE + " WHERE username = '" + username + "'";
                ResultSet resultSet = statement.executeQuery(selectQuery);
                if (resultSet.next()) {
                    // 用户名已存在，跳过插入操作
                    System.out.println("用户名 " + username + " 已存在，跳过插入操作。");
                    continue;
                }
    ...
    private static void insertMemoryData(Statement statement, String data) {
        try {
            String insertQuery = "INSERT INTO " + HISTORY_DATA_TABLE + " (info) " +
                    "VALUES ('" + data + "')";
            statement.executeUpdate(insertQuery);
